...
slicer_lambda is a Rust project that implements an AWS Lambda function in Rust.
...
```json
{
    "file": "s3://my-bucket/models/part.stl",
    "output_path": "s3://my-bucket/outputs/job-123",
    "config": {
...
Notes:
- `file` can also be `input`.
- `output_path` can also be `output`.
- If you omit `config`, defaults are used.
- Outputs are written to `output_path` as `output.gcode` and `stats.json`.
